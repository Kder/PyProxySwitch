...
def _import_pps_funcs():
    ...
    try:
        ...
        return pps_config.pps_load_proxylist, pps_config.pps_save_proxylist, pps_config.PROGRAM_PATH
    except ImportError:
...
class ConfigManager:
    ...
    def __init__(
        ...
        try:
            self.pps_load_proxylist, self.pps_save_proxylist, program_path = _import_pps_funcs()
    ...
    def _load_proxies(self) -> None:
        ...
        try:
            if not self.proxy_list_path.exists():
                ...
                self.logger.debug(f"代理列表已加载: {self.proxy_list_path} ({len(self._proxies)} 个代理)")
