...
class Window(QtWidgets.QDialog):
    ...
    def _load_translator(self) -> None:
        ...
        if not self.translator_qt.load(QtCore.QLocale(lang), "qtbase", "_", stdtranslator_path):
            get_logger().warning(
                f"QT base translator failed to load from: {stdtranslator_path}. "
        ...
        if not self.translator.load(translator_path):
            get_logger().warning(
                f"Translator failed to load from: {translator_path}. UI may not be translated."
            )
        else:
    ...
    def _initialize_tray(self) -> None:
        ...
        if not QtWidgets.QSystemTrayIcon.isSystemTrayAvailable():
            QtWidgets.QMessageBox.critical(
                ...
                self.tr("System Tray"),
                self.tr(
                    "I've not detected any system tray on this system.\nPyProxySwitch cannot start."
                ),
            )
            sys.exit(1)
    ...
    def showWelcome(self) -> None:
        ...
        self.trayIcon.showMessage(
            "Hi", self.tr("I'm here, welcome to PyProxySwitch!"), icon, 5 * 1000
        )
